"""歸因歷史（judgment_runs）：run 生命週期落庫 + 列表/詳情聚合 + 重判判定（合成列，免 LLM）。"""
...
def test_run_lifecycle_and_list(temp_db) -> None:
    ...
    db.insert_judgment_run(_run_row())
    db.update_judgment_run_status("pj_test01", "paused")
    got = db.list_judgment_runs()["items"][0]
    assert got["status"] == "paused" and got["kind"] == "batch" and got["finished_at"] is None

    db.finish_judgment_run(
        "pj_test01",
        {
            "status": "done",
            "processed": 3,
...
def test_list_source_filter_and_paging(temp_db) -> None:
    ...
    db.insert_judgment_run(_run_row(job_id="pj_a", source="product_reviews"))
    db.insert_judgment_run(_run_row(job_id="pj_b", source="conversations", kind="single"))
    assert db.list_judgment_runs(source="conversations")["total"] == 1
    assert db.list_judgment_runs(limit=1)["total"] == 2
    assert len(db.list_judgment_runs(limit=1)["items"]) == 1


def test_detail_aggregates_llm_usage_by_stage(temp_db) -> None:
    """詳情聚合同 job_id 的 llm_usage per-stage 明細（呼叫數/token/費用）。"""
    db.insert_judgment_run(_run_row(job_id="pj_det"))
    ...
    det = db.judgment_run_detail("pj_det")
    ...
    assert db.judgment_run_detail("pj_missing") is None


def test_any_judged_detects_rejudge(temp_db) -> None:
    """any_judged：標的已有判決→True（重判）；無判決/空清單/無來源→False。"""
