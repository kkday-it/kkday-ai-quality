...
def test_fake_generator_and_auditor_route(domain, plans_dir):
    slug = domain.lower().replace("-", "")
    plan = S.Plan(
        **json.loads((plans_dir / f"{slug}_layer1_plan.json").read_text(encoding="utf-8"))
    )
    cell = plan.cells[0]
            ...
            "cases": [
                {
                    ...
                    "evidence_quotes": ["现场细节足以判断"],
                    "label_reason": "决定性事实完整。",
                    "language": "zh-cn",
                    "pair_side": None,
                }
    ...
    def audit_reply(_system, _user, schema_name, schema, _n):
        ...
        assert schema["properties"]["suggested_l2_codes"]["items"]["enum"] == list(
            S.DOMAIN_L2_CODES[domain]
        )
        return {
            "label_supported": True,
            "ambiguous": False,
            "self_contained": True,
            "contains_independent_target_issue": False,
            "suggested_domain": "true",
            "suggested_l2_codes": [code],
            "evidence_quotes_valid": True,
            "near_duplicate": False,
            "pair_minimality_valid": True,
            "review_required": False,
            "audit_reason": "标签与证据一致。",
        }
...
def test_l2_pair_both_true_and_bound(plans_dir, domain):
    slug = domain.lower().replace("-", "")
    plan = S.Plan(
        **json.loads((plans_dir / f"{slug}_layer2_plan.json").read_text(encoding="utf-8"))
    )
    ...
    assert [c.expected_l2_codes for c in cases] == [
        [cell.target_l2_codes[0]],
        [cell.target_l2_codes[1]],
    ]
