...
def _finding(rec_oid: str, domain: str = "content", status: str = "new") -> TicketFinding:
    """建一筆對應 reviews 列的歸因（finding_id 依 fd_{source}_{source_id}__{domain} 慣例）。"""
...
def test_rejudge_preserves_human_status(temp_db) -> None:
    ...
    db.insert_source_batch("reviews", [_pr_row("R1")])
    fid = "fd_reviews_R1__content"
    db.replace_source_findings("reviews", "R1", [_finding("R1")])
    db.update_finding_status(fid, "confirmed")
    db.replace_source_findings("reviews", "R1", [_finding("R1", status="new")])
    assert _status_of(fid) == "confirmed"


def test_rejudge_preserves_dismissed_status(temp_db) -> None:
    """人工 dismissed 後重新初判仍保留。"""
    db.insert_source_batch("reviews", [_pr_row("R2")])
    fid = "fd_reviews_R2__content"
    db.replace_source_findings("reviews", "R2", [_finding("R2")])
    db.update_finding_status(fid, "dismissed")
    db.replace_source_findings("reviews", "R2", [_finding("R2", status="new")])
...
def test_rejudge_untouched_status_follows_new_judgment(temp_db) -> None:
    ...
    db.insert_source_batch("reviews", [_pr_row("R3")])
    fid = "fd_reviews_R3__content"
    db.replace_source_findings("reviews", "R3", [_finding("R3", status="new")])
    db.replace_source_findings("reviews", "R3", [_finding("R3", status="new")])
...
def test_rejudge_does_not_preserve_auto_confirmed(temp_db) -> None:
    ...
    db.insert_source_batch("reviews", [_pr_row("R4")])
    fid = "fd_reviews_R4__content"
    db.replace_source_findings("reviews", "R4", [_finding("R4")])
    db.update_finding_status(fid, "auto_confirmed")  # 系統自動確認
    db.replace_source_findings("reviews", "R4", [_finding("R4", status="new")])
