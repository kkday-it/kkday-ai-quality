...
| `prompt_debug.py` | 售後根因 Prompt 調試：Google Doc 分類 SSOT 渲染、strict Structured Outputs、SSE token 串流、category 相依欄位校驗與單次 usage/費用估算；只記 `llm_usage`，不污染正式 judgments/history。 |
