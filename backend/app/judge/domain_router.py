...
本模組以系統自己的歷史判決為訓練資料（scripts/tools/train_domain_router.py 離線訓練），
...
def _router_cfg() -> dict:
    """讀 judgment.json prejudge.domain_router 區塊（熱重載跟隨 prejudge 配置快取）。"""
...
def _dom_pid_map() -> dict[str, str]:
    ...
    key 用 `prompt_source._domain_of`（檔名尾綴）——與 judgments.l1_code / gated attrs 的
...
def decide(text: str, polarity: str) -> RouterDecision:
    ...
    Args:
        text: 判決主輸入文字（prejudge._text_of 產出）。
    ...
    except Exception:  # noqa: BLE001  路由是省錢輔助，任何故障都不得阻斷判決（fail-open）
...
def report_shadow(
    ...
    *,
    ...
    """影子比對：全跑產出的採信歸因 vs 路由候選——漏域即路由假陰性，落 judgment_history 留痕。

    kind='router_shadow'（Text 欄新增邏輯值零 migration，比照 kind='failure' 模式）；params 記
    {candidates, missed, probs}。best-effort：寫入失敗僅 log 不拋，絕不阻斷判決。
    """
    try:
    ...
    except Exception:  # noqa: BLE001  影子留痕是量測輔助，寫不進去也不能拖垮判決
