...
| `routers/problems.py` | 統一問題列表 / 縱覽聚合（/api/problems*，皆支援 model CSV 多選＝判決模型篩選·當前判決維度）+ 列表導出（POST /api/problems/export → 背景 job；欄尾附 C-1~C-6 六域命中欄〔符合/不符合·未判留空〕＋「分類統計」「Prompts」工作表；`snapshot_model`＝輸出結果版本〔內容替換為該模型 judgment_history 最新快照〕；`compare_models`＝並排對比模型多選〔基準右側每模型附一組情緒/L1/L2 欄，值取該模型最新快照〕——兩者語義獨立可並用）。 |
...
| `routers/rules.py` | 判決規則版本化 CRUD（/api/judge-rules：list/active/history/save/restore/reset + jsonschema 驗證）；POST `/export` 啟動判決 Prompt 包 zip 導出背景 job；Prompt 草稿（GET `/drafts` 草稿存在清單、GET/PUT/DELETE `/{code}/draft` 單支草稿——未入庫編輯中內容，供沙盒送測/雙跑對比，見 `core/db/prompt_drafts.py`；存草稿寬鬆只驗 text 非空，送測/入庫才強驗）；POST `/{code}/validate` dry-run 驗證 prompt md（不落庫，內容不合法回 200 `{valid:false,error}`）。**寫入端點（save/restore/reset×2）掛 `require_permission(judge-rule.version.manage)`（admin 級·403）**。 |
