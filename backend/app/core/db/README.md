...
| `prompt_drafts.py` | 判決 Prompt 草稿（prompt_drafts；prompt_* 每 rule_code 一份共享草稿＝未入庫的編輯中內容）：沙盒可直送測（雙跑對比），滿意後走 `save_rule_version` 入庫並刪草稿；與 judge_rule_versions 分離（版本表維持「存檔即 active」單一語意），併發 last-write-wins。 |
...
| `export.py` | 問題列表美化 xlsx 導出（1:N fan-out + review 級欄合併儲存格；polarity 整列底色正綠/中灰/負紅；行高顯式鎖定為排除長文欄（評論內容/商品名稱/方案名稱）後各欄所需高度；資料欄尾附 **C-1~C-6 六域命中欄**〔`_domain_match_cols` 取 `prompt_source.structure()` SSOT·review 級合併·值＝符合/不符合·未判留空·供 Excel 篩選〕；另附「分類統計」圖表表（見 `export_stats.py`）與「**Prompts**」工作表〔`_append_prompts_sheet`：7 支判決 prompt active 版本快照·版本 meta 取 `list_rule_meta`·全文 DB active 優先/檔案回退·判決溯源〕；`snapshot_model`＝輸出結果版本：內容/列傾向替換為該模型 judgment_history 最新快照〔`_adapt_snapshot`·覆核軸留空·未判過的評論排除·口徑寫統計表 A2〕；`compare_models`＝並排對比模型多選：基準右側每模型附一組 review 級欄「情緒·M/L1·M/L2·M」〔`_compare_cols`/`_compare_values`·值取該模型 `latest_snapshots`·鍵前綴 `cmp__{model}__*` 不撞 attr 級鍵故自動合併儲存格·未判/判為無問題該欄空白〕）。 |
| `export_stats.py` | 導出分類統計（由 in-memory rows 直接算情緒傾向/L1/L2/判決分層/判決階段分佈，附「分類統計」表；≤6 類圓餅、>6 類橫向長條）。所見即所得。 |
