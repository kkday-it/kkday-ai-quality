...
| `rule_versions.py` | 初判規則版本化（judge_rule_versions；active/歷史/恢復默認/seed）。`RULE_CODES`＝bd_tag_vertical + source_mapping 兩類；**7 支初判 Prompt 已於 2026-07-28 遷出至檔案版本庫**（`prompts/<id>/v*.md` ＋ `ACTIVE`，見 `app/judge/prompt_versions.py`），不再由本表管理。 |
