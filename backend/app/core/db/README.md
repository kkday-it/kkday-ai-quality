...
| `dimensions.py` | 值域主檔（`attribution_dimension_master`）：**四軸**共用一表，以 `dimension_code` 判別（同 `judge_rule_version_lst` 的 `rule_code` 慣例）。前三軸屬判決歸因（責任方／嚴重度／建議行動），`note_type` 是備註的互動類型——語義不同但欄形完全相同，拆表只為語義分組不值得再付一套 migration／API／畫面。種子取自既有 `Severity`／`RecommendedAction` Literal 與各域 `_meta.owner_role`，檔案 `config/ai_judge/attribution_dimension.json` 為 seed、本表存 live。**無刪除函式**：停用走 `is_active=false`（硬刪已被歷史引用的 code 會讓那些列顯示空白）。 |
