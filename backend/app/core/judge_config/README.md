...
讀 `config/ai_judge`、`config/global` 的 JSON，提供初判引擎所需的判準/流程/映射。各 loader lazy load +
...
| `rule_export.py` | `app.judge.prompt_source.structure()` + prejudge.json | 6 域面向結構 + 初判配置 Excel 導出 + `_style_header`（品牌樣式，db.export 複用）|

> `ai_judge.py`/`rule_export.py` 判準結構讀 `app.judge.prompt_source.structure()`（Prompt-as-Source），不讀 DB 規則樹；判準文字 100% 在 `prompts/*.md`。初判/判決流程設定集中 `config/ai_judge/prejudge.json`＋`verdict.json`（靜態設定檔，後端合併載入，改值需重啟後端）：`prejudge.py` 經 `_cfg()` 讀取 `_evidence_policy()`/`_polarity_gate_cfg()`。
