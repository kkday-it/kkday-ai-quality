"""graceful shutdown 收尾：彙總標記 5 套 in-mem job registry 的進行中 job 為 interrupted。
...
def mark_running_jobs_interrupted() -> dict[str, list[str]]:
    """把 5 套 registry（export/import/prejudge/upload/prompt_sandbox）仍在跑的 job 全標 interrupted。
