...
- taxonomy：圈號↔code 橋接、候選域過濾（⑤⑦）、confidence 封頂。
- 本模組：提供 L3 的「判準內容」（taxonomy 的 attribution_tree 只有結構與 severity，無 canon）。

domain ↔ 圈號 橋接讀 config/ai_judge/domains.json（單一真相源），與 rule 檔 _meta.domain 對齊。
...
from app.core.paths import TAXONOMY_DIR as _TAXONOMY_DIR


def _domains_file() -> Path:
    """domains.json 路徑：config/ai_judge 優先，回退 config/taxonomy（保相容未遷移環境）。"""
    aj = _AI_JUDGE_DIR / "domains.json"
    return aj if aj.is_file() else _TAXONOMY_DIR / "domains.json"
...
# domains.json 完整 meta（code → {label_zh/intake_excluded…}）；漏斗 Stage1 選域用
...
def selectable_domains() -> list[dict[str, Any]]:
    ...
    每域附中文名 + L2 面向涵蓋，讓 LLM 先在域層聚焦選擇（對抗一次攤平 60+ 條 L3 的注意力稀釋）。
    不可抗力 / 客服營運不在預判選域（intake_excluded，逃生改走 polarity + L1→L3 歸因）。

    Returns:
        [{code, label, l2_labels}]，依 domains.json 原始順序；不含 intake_excluded 域。
    """
    _ensure_loaded()
    out: list[dict[str, Any]] = []
