...
| `auth_verifiers.py` | be2 模式登入 verifier（去帳戶系統後僅 `authProvider=be2` 才介入·local 模式不經過此模組）：`get_verifier()` 回 `Be2TokenVerifier`（be2 accessToken claims decode＋exp＋email 自動 provision 方案 A；**驗簽 TODO 待 auth team 契約·production 啟用即拒**，`is_production()` 硬閘拋 RuntimeError）。 |
