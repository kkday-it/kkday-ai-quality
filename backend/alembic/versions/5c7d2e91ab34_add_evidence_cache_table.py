...
- evidence_cache：qc_evidence 兩級快取的 KV 儲存（cache_key PK / kind / payload JSONB /
...
def downgrade() -> None:
    """回滾：整表移除（純快取，無資料保留需求）。"""
    op.execute("DROP TABLE IF EXISTS evidence_cache")
