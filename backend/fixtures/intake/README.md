...
## 欄位（30 欄扁平 CSV，含大寫 `PM`）

其中 29 欄逐字對應 `conversation_tbl` 欄名；`product_category` **無對應 DB 欄**（不在
`source_registry.header_column_map("conversations")` 內），上傳時會被列入「未對應表頭」回報而不入庫。
