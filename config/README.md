...
## global/（業務可調·跨環境）
- `llm_model.json` — LLM providers 目錄 + defaultModels + per-model 單價（pricing loader / 前端下拉共用）+ 根層 `embeddings[]`（embedding 模型單價，域路由特徵用——僅計價、不進聊天模型下拉）+ 根層 `areas[]`（LLM 消費功能區清單：prejudge/prompt_debug/sandbox/prompt_revise，`llm_area_defaults` 的 key 集合）+ 根層 `areaDefaults`（**該區還沒存過團隊默認時的出廠起點**，如 `prompt_revise` 直接起在旗艦 `gpt-5.5` + high effort——各區合適的模型檔次差很多，裁決跑批要便宜、改 Prompt 要聰明，不能全共用一組；後端 `settings.area_seed_knobs()`、前端 `LLM_AREA_SEEDS` 同讀此欄，未登記的區維持原本的空白起點）+ `modelCapabilities`（per-model 可配參數能力覆寫；未登記者回退所屬 provider 的 `supportsThinking`/`reasoningEffortOptions`/`temperatureLockedWhenThinking`/`lockedTemperatureValue` 欄位，後端 `settings.model_capabilities_for()` 讀取）
...
- `verdict.json` — **判決階段**配置（同為靜態設定檔，與 `prejudge.json` 合併載入）：**auto_confirm（G1 自動確認路由：enabled + audit_sample_rate，系統判決留痕 verdict_by='system:auto_confirm'）** + 判決狀態中文 label（`status_labels`：new=待判決/auto_confirmed=自動確認/confirmed=已確認/dismissed=已駁回；後端 `_shared._STATUS_LABEL_ZH` 有 code-side fallback 容忍缺鍵）+ 判決人中文 label（`verdict_by_labels`：system:auto_confirm=系統自動確認；人工判決存操作者 email 不查表、原樣顯示；後端 `_shared._VERDICT_BY_LABEL_ZH` 有 code-side fallback，導出 xlsx「判決人」欄套用）。
- `evidence.json` — **訂單佐證（qc_evidence）子系統**旋鈕（靜態設定檔，純後端消費、前端不讀；改值重啟生效）：db（dbname/schema/connect·statement timeout/pool_size 併發上限/熔斷 threshold·cooldown）+ cache（order_ttl_hours 6·一訂單一列 PK=order_oid 的單一 TTL、易變欄位短 TTL（快取落 PG evidence_snapshot 表，拆欄——ID/純量各自成欄、商品/規格/方案內容各自獨立 jsonb 欄，非單一 payload blob，TTL 懶清理））+ singleflight_wait_timeout_s + summary（lang_fallback／max_chars_per_field／max_total_chars——`prejudge._summarize_evidence` 摘要器截斷旋鈕，輸出兼作 evidence_citation 落庫）。`enabled=false` 一鍵停用佐證取數（判決全走無佐證降級）。
- `after_sales_root_cause.json` — **售後根因分類庫 SSOT**（`app.judge.prompt_debug` 唯一消費者，供輸出校驗與裁判表摘要；調試台/跑批實際送出的 system prompt 走 `prompts/debug/after_sales_root_cause/` 最新時間戳快照）：5 themes × 25 categories（`definition`/`include`/`exclude`/`examples`/`likely_causes`/`product_opportunity`/`vertical`）+ `modify_target_options` + `oot_subtype_options`。部分 category 另附 `calibration`（實測校準判例；與 Definition/Include/Exclude 字面衝突時以它為準，模板已宣告此優先序）——目前有 C03 商品規格/使用規則事前確認（2026-07-27 併入：判準＝用戶問的標的「規則條文怎麼適用」，非客服答不答得出；2026-07-28 增第四種入口：取消事由被客服證實為誤解→本類）、C06 取消政策本身僵化（2026-07-28 增前置例外：事由被證實為誤解者轉 C03）、C07 取消政策揭露不清（2026-07-27 併入：6 vs 7 決勝軸＝商品客觀可退性）、C12 修改受限、C13 特殊需求/加購五類，其餘校準仍留在 prompt 靜態快照待轉正式時併入。
