...
- `after_sales_root_cause.json` — **售後根因分類庫 SSOT**（調試台 v2 契約專用；`app.judge.prompt_debug` 唯一消費者）：5 themes × 25 categories（`definition`/`include`/`exclude`/`examples`/`likely_causes`/`product_opportunity`/`vertical`）+ `modify_target_options` + `oot_subtype_options`。部分 category 另附 `calibration`（實測校準判例；與 Definition/Include/Exclude 字面衝突時以它為準，模板已宣告此優先序）——目前僅 C12 修改受限、C13 特殊需求/加購兩類有，其餘校準仍留在 v3 靜態快照待轉正式時併入。
