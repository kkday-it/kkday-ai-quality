// 判決規則狀態（product_vertical/global_rule/source_mapping + prompt_* 的 active content / dirty / 歷史 / 存檔恢復）。
...
export const useJudgeRulesStore = defineStore('judgeRules', () => {
  ...
  const activeCode = ref<RuleCode>('global_rule'); // 當前選中子規則
  ...
  /** 恢復整體配置（global_rule + source_mapping）為檔案默認，各新增版本；重載清單與當前選中，回傳 {reset, skipped}。 */
