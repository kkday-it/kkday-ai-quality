...
/** 要驗哪一份：有候選（套過補丁）時預設驗候選，否則只能驗現行。 */
const target = ref<'candidate' | 'baseline'>('baseline');
watch(
...
const targetPrompt = computed(() =>
  target.value === 'candidate' && props.candidatePrompt
    ? props.candidatePrompt
    : props.baselinePrompt,
);
const canRun = computed(
  () => !regression.running.value && props.reviewIds.length > 0 && !!llm.knobs.model,
);
