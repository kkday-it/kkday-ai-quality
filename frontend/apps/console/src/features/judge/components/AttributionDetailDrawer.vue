    ...
    <div v-if="row" class="flex flex-col gap-4">
      ...
      <template v-if="row.attributions && row.attributions.length">
        ...
        >
          ...
          <a-descriptions-item v-if="a.evidence_status" label="佐證留痕">
            <a-tag size="small" :color="EVIDENCE_STATUS_COLOR[a.evidence_status] || 'gray'">
              {{ EVIDENCE_STATUS_LABEL[a.evidence_status] || a.evidence_status }}
            </a-tag>
            <span v-if="a.evidence_fetched_at" class="ml-1.5 text-xs text-[var(--color-text-3)]">
              {{ fmtDt(a.evidence_fetched_at) }}
            </span>
            <!-- 判決當時注入 prompt 的佐證摘要（歷史快照，非上方即時佐證）-->
            <a-typography-paragraph
              v-if="a.content?.evidence_citation"
              class="mb-0 mt-1 text-xs text-[var(--color-text-2)]"
              :ellipsis="{ rows: 2, expandable: true }"
            >
              {{ a.content.evidence_citation }}
            </a-typography-paragraph>
          </a-descriptions-item>
