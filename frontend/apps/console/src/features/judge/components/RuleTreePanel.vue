...
/** 深度優先找第一個 L3 葉 code（預設選中用）。 */
function firstL3Code(nodes: Node[]): string | null {
  for (const n of nodes) {
    if (n.level === 3) return n.code;
    if (n.children) {
      const c = firstL3Code(n.children);
      if (c) return c;
    }
  }
  return null;
...
watch(
  ...
  (c) => {
    ...
    const first = firstL3Code(roots.value);
...
const selectedNode = computed<Node | null>(() => {
  ...
  return n && n.level === 3 ? n : null; // 只編 L3 葉
});
...
<template>
  <div class="flex h-full gap-4">
    ...
    <div class="h-full min-w-0 flex-1 overflow-auto rounded-lg border p-3">
      ...
      <a-empty v-else description="選一個 L3 細項節點編輯判準" />
    </div>
  </div>
</template>
