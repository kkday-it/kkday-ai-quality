...
const kpi = computed(() => {
  ...
  const lowConf = all.value.filter(
    (f) => f.status === 'new' && f.confidence < 0.7 && CONTENT.has(f.verdict),
  ).length;
  return {
    total: t,
    contentPct: t ? Math.round((content / t) * 100) : 0,
    topDim: top ? top[0] : '—',
    topN: top ? top[1] : 0,
    lowConf,
  };
...
const mtx = computed(() => {
  ...
  DIMS.forEach((d) => {
    m[d] = {};
    VKEYS.forEach((v) => (m[d][v] = 0));
  });
  all.value.forEach((f) => {
    if (m[f.dimension]) m[f.dimension][f.verdict]++;
  });
  return m;
});
...
const option = computed(() => ({
  ...
  series: [
    {
      type: 'heatmap',
      data: DIMS.flatMap((d, di) => VKEYS.map((v, vi) => [vi, di, mtx.value[d][v]])),
      label: { show: true },
    },
  ],
}));
...
const openDimDrawer = (d: string) => {
  drawerDim.value = d;
  drawerKind.value = 'dim';
  drawerVisible.value = true;
};
    ...
    <div>
      <a-row :gutter="14" class="mb-4">
        ...
        <a-col :span="6"
          ><KpiCard
            label="確認為內容問題"
            :value="kpi.contentPct"
            unit="%"
            subtext="點擊查看內容問題 →"
            @click="openDrawer('content')"
        /></a-col>
        <a-col :span="6"
          ><KpiCard
            label="最痛維度"
        ...
        <a-col :span="6"
          ><KpiCard
            ...
            :value="kpi.lowConf"
            subtext="點擊查看 →"
            @click="openDrawer('lowconf')"
        /></a-col>
      </a-row>

      <CardSection
        title="維度 × verdict 熱力矩陣（概覽）"
        hint="問題分布熱點概覽 · 查看明細請用下方「問題明細」篩選器"
        class="mb-4"
      >
      ...
      <CardSection
        title="⚠ 規則缺口"
        hint="高頻缺漏/模糊但無對應規則 → 補 rules.json 閉環"
        class="mb-4"
      >
        ...
        <template #extra>
          <a-space wrap>
            ...
            <a-select
              ...
              @change="doSearch"
            >
              <a-option v-for="(l, i) in VLABEL" :key="VKEYS[i]" :value="VKEYS[i]">{{
                l
              }}</a-option>
            </a-select>
            <a-select
