...
// label SSOT＝repo 根 config/ai_judge/judgment.json status_labels（前後端同讀；與 TIER/STAGE 同慣例）。
import judgment from '@config/ai_judge/judgment.json';

/** 處理狀態 → 中文 label。auto_confirmed＝G1 系統自動確認（高信心免人工）；fixed 已撤除。 */
export const STATUS_LABEL: Record<string, string> = judgment.status_labels;
