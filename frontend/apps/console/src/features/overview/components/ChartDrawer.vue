...
const table = computed<{
  ...
  if (t === 'trend') {
    ...
    const columns = [
      { title: '序列', dataIndex: 'k' },
      ...td.months.map((m, i) => ({ title: m, dataIndex: `v${i}` })),
    ];
  ...
  if (t === 'funnel') {
    ...
    return {
      columns: [
        { title: '階段', dataIndex: 'name' },
        { title: '留存', dataIndex: 'value' },
      ],
      rows: fd.stages.map((s) => ({ name: s.name, value: `${s.value}${fd.unit}` })),
    };
  }
  if (t === 'coverage') {
    const rows = (d as CategoryCoverageRow[]).map((r) => ({
      name: r.prod,
      t2: r.tier2,
      t3: r.tier3,
    }));
    return {
      columns: [
        ...
        { title: 'Tier2', dataIndex: 't2' },
        { title: 'Tier3', dataIndex: 't3' },
      ],
      rows,
    };
  }
  ...
  if (t === 'gauge') {
    ...
    return {
      columns: [
        { title: '當前', dataIndex: 'v' },
        { title: '目標', dataIndex: 'target' },
        { title: '上限', dataIndex: 'max' },
      ],
      rows: [
        {
          v: `${g.value}${g.unit}`,
          target: g.target != null ? `${g.target}${g.unit}` : '—',
          max: `${g.max}${g.unit}`,
        },
      ],
    };
