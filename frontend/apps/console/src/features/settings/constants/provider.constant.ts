// LLM 供應商定義與設定面板常數。
// 供應商目錄 / reasoning 選項的「資料」來自 repo 根 config/global/default_llm.json（跨語言共用單一真相源）；
// 本檔僅保留型別與前端衍生預設，不再寫死 base_url / model 清單。

import llm from '@config/global/default_llm.json';
 ...
 * 資料源＝config/global/default_llm.json（GPT model id 對齊 OpenAI 官方 gpt-5.5 / gpt-5.4 / mini / nano）。
 * JSON 字面型別與 Provider[] 結構相容（api_token 選填、不在 JSON 中），以 cast 收斂。
 */
export const PROVIDERS = llm.providers as Provider[];

/** Reasoning effort 對齊 OpenAI 官方支援值；資料源＝config/global/default_llm.json。 */
...
/**
 * LLM 設定面板表單的「UI 展示預設」。衍生自 openai 供應商 preset，避免與 config/global/default_llm.json 重複寫死。
