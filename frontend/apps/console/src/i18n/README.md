...
<template>{{ t('common.app.name') }}</template>
```
...
- 逐 feature 試點挖字；原 pilot（`auth` login 頁 + `AUTH.*` error code）已隨去帳戶系統（2026-07-22）
  一併退役——本地模式無登入頁，下個 pilot 待選；其餘 feature touch-when-edit 逐步遷移，各自獨立可 revert。
