...
| 檔 | 職責 |
|---|---|
| `loader.ts` | **唯一可替換接縫** `loadLocaleMessages(locale)`：現以 Vite `import.meta.glob` 靜態讀 `locales/<locale>/*.json`（filename=namespace）；日後接 TMS 只改此函式（改 fetch），i18n instance / `$t` / 元件全不動。async 簽名預留。 |
...
| 判準領域 label（tier / polarity / stage·DB 存 code·QC 可調） | **留** `config/ai_judge/judgment.json` / `constants/labels`（不挖） |
| debug / 內部代碼 | 留碼內 |
...
## 換翻譯來源（日後·唯一改動點）
`loader.ts::loadLocaleMessages` 改為 fetch TMS（回傳同形狀 namespace map）；`setupI18n` / `$t` / 所有元件不動。env `VITE_I18N_SOURCE` 可預留分支（YAGNI·未實作）。
