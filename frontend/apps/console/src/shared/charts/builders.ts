/**
 * 共用 ECharts option 建構器（純函式，無副作用）——overview / judge / usage 三 feature 同用。
 * 元件只負責把 data 餵進對應 builder 並渲染 <v-chart>，配色 / 結構集中於此單一真相。
 * 圖表型別的 `use([...])` 註冊由消費頁面統一執行（見 DashboardView.vue），此處不耦合 echarts/core。
 * 原 features/overview/utils/overviewCharts.ts；因被 judge/usage 反向依賴而下沉 shared（原路徑 re-export）。
 */
import type { TrendData, IntakeBreakdown, ReviewFunnel, GaugeData, BarData } from './chart.types';
...
/**
 * 占比分布甜甜圈：各類別占比，中心留白可置標題。
 * @param data 分布資料（各 item 自帶語義色）
 */
