...
```bash
./scripts/dev/start.sh                                        # 一鍵：自動裝引擎（colima）→ up（前景，Ctrl-C 停）
docker compose -f docker-compose.dev.yml up -d                # 背景啟動
./scripts/dev/stop.sh                                         # 停止（資料保留）；--wipe 連 pgdata_dev 一起刪
...
./scripts/dev/stop.sh --wipe                                           # ⚠️ 刪 pgdata_dev＝資料庫全清（重來全新空庫）
...
| 想全部砍掉重來 | `./scripts/dev/stop.sh --wipe` → `./scripts/dev/start.sh`（空庫；資料靠 seed 或前台資料包匯入）|
