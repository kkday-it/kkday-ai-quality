...
```bash
./start.sh prod    # 首次自動生成 POSTGRES_PASSWORD / AIQ_JWT_SECRET / AIQ_SECRET_KEY 寫入 repo 根 .env
...
| `AIQ_JWT_SECRET` | JWT 簽章（≥32 bytes；生成 `python -c "import secrets;print(secrets.token_urlsafe(32))"`） | compose / backend 拒啟動 |
| `AIQ_SECRET_KEY` | user_settings 機密 at-rest 加密（生成同上） | compose / backend 拒啟動 |
| `OPENAI_API_KEY` | LLM 判決 key（fallback；亦可登入後於設定面板配 per-provider token） | 可啟動，但 **stub 硬閘**：批量判決一律 403（防假判覆蓋真歸因） |
| `CORS_ALLOW_ORIGINS` | 前端正式網域（逗號分隔多個） | 預設 `http://localhost:8080`，上線必改 |

```bash
./start.sh prod                    # 一鍵（自動生成缺漏機密 → build → 等就緒）；前端 http://localhost:8080
POSTGRES_PASSWORD=... AIQ_JWT_SECRET=... AIQ_SECRET_KEY=... OPENAI_API_KEY=... \
