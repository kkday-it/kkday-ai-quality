...
**5. 臨時內容不入版本庫，保持 repo 純潔。** 一次性實驗腳本（A/B 對比、單次遷移、調參試跑）與實驗產物（評測結果、中間數據、分析報告）**禁止 commit**——一律放 `tmp/` 或 `/data/`（皆已 gitignore）；要留存的結論寫進正式文檔或交付報告，不以腳本殘骸形式留在 repo。只有**可重複使用**的工具（週期性重訓、通用評測閘門、維運操作）才進 `scripts/` 並登記於 `scripts/README.md`。既有腳本一旦使命完成（遷移已跑完、依賴已退役）→ 視同死碼，依核心原則 4 清退（含 README 條目）。
