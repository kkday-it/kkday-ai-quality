...
### 4-b. 抽屜／彈窗內的表格：可用寬遠小於容器寬（2026-08-11 實測）

`a-drawer :width="640"` 內的表格，**欄寬總和 580 仍會出現橫向捲軸，488 才不會**。
差距來自 drawer body padding ＋ StickyTabs 內距 ＋ 表格外框，加起來比直覺估的多。

- **起手就抓「容器寬 × 0.76」以下**，別從「容器寬 − padding」開始試——那個估法錯兩次了
- **總和壓低不吃虧**：Arco 於總和 < 容器時等比拉伸，比例才是決定性的；壓低只是把
  「拉伸率 > 1」而已，各欄相對關係不變
- **沒宣告 width 的欄會被擠死**：它只分到「容器 − 其餘欄總和」。實例：`NoteTypePanel` 的
  說明欄沒給 width，在 640 抽屜裡只分到約 60px，**連表頭都被裁成一個字**。
  內容欄一律明確給權重，不要留空讓它「自適應」
