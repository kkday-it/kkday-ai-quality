...
> 本專案既有代碼註釋密度高（如 `judge/llm/client.py` 的 `ping` / `judge/prejudge.py` 各閘門皆有「為何」註解）——新增 / 修改代碼比照既有密度，勿降低。

## 重庫 lazy import（預設機制）

重量級 / 選用依賴（openai / 大型 SDK / 資料處理庫等）**不在 module 頂 import**，改在實際用到的函式內 import，加速啟動並降低未用路徑的載入 / 記憶體成本：

- 判準：**每次 import 本模組都需要它嗎？** 否（僅特定函式 / 選用功能用）→ 函式內 import（見 `judge/llm/client.py` 的 `from openai import OpenAI` 置於 `_get_client()` 內）
