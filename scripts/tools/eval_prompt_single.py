...
def _sample_polarity(n: int, source: str) -> list[dict]:
    ...
    否則三態各 n/3（md5 穩定），帶 production polarity/sentiment 真值。golden 對極性無真值來源，
    沿用 production（true_label 為 L1/L2 域碼，非 polarity 三態）。
    """
    if source == "mock":
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="參照集：production（現行判決）/ golden（true_label 人審真值）/ mock（prompt_testcases 邊界測試集）",
    )
