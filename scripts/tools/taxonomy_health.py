"""歸因分類體系健檢（多模型交叉診斷）——不依賴人工真值，用多模型初判的一致性與分歧結構反推分類體系品質。

核心邏輯（誠實定位）：全庫幾無人工真值，無法算 accuracy。改用多個獨立模型
（gpt-5-mini / ByteDance seed / Gemini / Claude，皆判同一批商品評論、快照存 attribution_history）
...
不歸因）；L3 層因初判深度=l2 不判故不評；多模型皆封閉式判（被餵當前樹），能診斷「現有類好不好用」
...
# 預設納入對比的模型（attribution_history 現有的四模型；gpt-5-mini 為生產主初判）
_DEFAULT_MODELS = [
    "gpt-5-mini",
...
def build_report(models: list[str], source: str) -> str:
    ...
    L.append(
        "\n## 方法論與侷限（先讀）\n"
        "- **這不是準確度**：全庫幾無人工真值，無法算 accuracy。本報告用多個獨立模型初判的"
    ...
    L.append(
        ...
        "- **L3 深度**：當前初判深度=l2，L3 節點全未觸發——非分類缺陷，是初判設定；"
        "若要評 L3 品質需先開 l3 深判。"
    )
