...
| `./scripts/ops/retention.py` | **資料保留期清理**（維運週期性）：依 `config/global/retention.json` 清掉過期的觀測型資料——`llm_usage_lst` 逾期列 / `attribution_event_lst` 已被取代的 failure 與每組超額的 prejudge 快照 / 逾期跑批產物目錄。**預設 dry-run 只報不刪**，`--apply` 才執行；`--only` 可挑項目。不碰 note 事件、當前初判（`attribution_tbl`）、版本庫（`judge_rule_version_lst`）與 5 張來源鏡像表 | `docker compose -f docker-compose.dev.yml exec backend python /app/scripts/ops/retention.py` |
...
| `./scripts/tools/multi_model_eval.py` | 多模型準確度評測（唯讀不寫 `attribution_tbl`）：`--build-set` 建評測集（有外部 free_tag 且已初判的 reviews）／`--run --area prejudge --provider <id> --model <id>` 以指定功能區默認 + 覆寫供應商/model 逐則 `to_findings` 收集 sentiment/polarity/歸因 | `cd backend && .venv/bin/python ../scripts/tools/multi_model_eval.py --help` |
| `./scripts/tools/persist_multimodel_history.py` | 把多模型評測結果（`tmp/multi_model/*_v4.json`）灌進 `attribution_event_lst`（kind='prejudge' 每評論每模型一筆快照，**只寫歷史不碰 `attribution_tbl` 活表**→列表恆 gpt canonical）：供歸因歷史 modal 看各模型、導出 `compare_models` 並排對比；去重天然（model+params+digest）、label 由 BD/Gemini 自帶＋ai_judge fallback 補 | `docker exec kkday-ai-quality-backend python /app/scripts/tools/persist_multimodel_history.py --dry-run` |
...
| `./scripts/tools/eval_gold_set.py` | **n=200 金標評測閘門**：對 200 則分層抽樣反饋（8 個獨立標註者盲標 + 4 個複核，1★45/2★30/3★45/4★35/5★45）跑完整六域，量**真實準確率**。與 `eval_prompt_single.py` 互補——後者的參照集是 production attributions（舊 prompt 的產物），修好覆蓋盲區會被算成「多報」、顯示為退步；本工具的參照集是人工金標，量「判得對不對」。**每則金標帶 anchor 逐字片語，開跑前驗證它真的出現在原文中，任一則對不上就整批拒跑**——2026-08-10 曾有 3 則金標的判準理由與原文完全不符，導致 prompt 被朝錯誤標籤優化。輸出含樣本準確率、排除待拍板、**母體加權準確率**（依星等權重還原 39,649 則母體）與各星等拆解；逐則結果落 `tmp/gold200_last_run.json`。⚠️ n 決定可判讀性：n=30 時 95% CI 達 ±13pp，任何 prompt 效應都被抖動蓋掉；n=200 收到 ±2.9pp。 |
| `./scripts/tools/verify_coverage.py` | **覆蓋歸屬回歸鎖**：對已定案的跨域場景（贈品破損／降規格／重複扣款／保證成團…）跑完整六域，斷言**主歸因**（`is_primary` 那條）落在拍板的域。補 `eval_prompt_single`（參照集下棄權率與多報率已觸頂觸底、測不到覆蓋）與 `eval_equivalence`（只比前後差異、不驗歸屬正確性）的缺口，同時擋「無人接」與「收錯／雙收」——兩者在聚合指標上都看不出來。⚠️ 真打 LLM，改完 prompt 且**發完 DB 版**後手動跑（未發版會測到舊版且不報錯）。新增覆蓋定案時要在 `CASES` 登記，該定案才有回歸保護。 |
| `./scripts/tools/audit_abstained.py` | **靜默漏判抽查**（唯讀·零 LLM 成本）：撈「判了非正向、六域卻全部棄權」的反饋。補的是一個**結構性**盲區——多模型一致率對這池子的敏感度是零（全部棄權＝三方 100% 一致，永遠不進分歧清單），而 `taxonomy_health.py` 自述只能診斷「現有類好不好用」、答不了「缺什麼類」。這池子的真值是「六域皆棄權」，故**任何從棄權變成有歸因的案例，數學上不可能被算成多報**，人只需判「這條歸因對不對」。另印主題分布縮小人工抽查範圍——⚠️那是**提及率不是漏判率**（詞表不分褒貶，「導遊很棒」同樣命中「人員服務」），不可直接讀成「該主題有 N 則漏判」；2026-08-12 實測 207 則有原文者：人員服務 48%、時間節奏 30%。⚠️ 過濾條件是「非正向 × 無 l1_code」，不是 `prejudge_stage='pending_data'`（後者全庫僅個位數）；孤兒歸因（來源列已被抽換）另計不混入分母。 | `docker compose -f docker-compose.dev.yml exec -T backend python /app/scripts/tools/audit_abstained.py --n 40 --out /app/tmp/abstained.json` |
