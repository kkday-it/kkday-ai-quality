...
| `scripts/prompt_lab/`（模組）| **C-1 單域 prompt Mock 評測實驗室**（隔離·OpenAI Responses API·**不碰生產 prejudge/DB/前端**）：`build_plans`/`build_manifest` → `generate_cases` → `audit_cases` → `build_dataset` → `evaluate_prompt`(+`metrics`/`report`) → `compare_runs`，即「生成→獨立審核→人工複核→冻結 Dev/Holdout→跑 baseline→逐條錯誤→換候選 prompt→diff」。有別於上兩列（服務舊六域競爭式分類）。用獨立 venv `.venv-promptlab`。完整說明見 [`evals/prompt_lab/README.md`](../evals/prompt_lab/README.md) | `.venv-promptlab/bin/python scripts/prompt_lab/evaluate_prompt.py --help`；測試 `.venv-promptlab/bin/python -m pytest backend/tests/prompt_lab` |
