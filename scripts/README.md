...
| `./scripts/tools/eval_equivalence.py` | **管線等價性閘門**（升級計畫 P0）：`--build-golden` 建金標集（judgments 分層 md5 穩定抽樣）／`--run <tag> --user <email>` 金標集全量走 production `to_findings`（不落庫·強制關 exact-cache 讀取）／`--compare A B` 出等價指標報告（噪音地板法：基線雙跑＝地板，改動 run 各指標 ≥ 地板 −1pp 才過閘） | `docker exec kkday-ai-quality-backend python /app/scripts/tools/eval_equivalence.py --help` |
| `./scripts/tools/train_domain_router.py` | **域路由器離線訓練**（升級計畫 P3）：歷史判決當標註 → embedding + per-domain LogisticRegression → holdout recall ≥0.995 選閾值（不達標/正樣本不足 → always_on）；產 `data/router/weights.json` + 訓練報告 | `docker exec kkday-ai-quality-backend python /app/scripts/tools/train_domain_router.py --user <email>` |
