...
`ops/`（生產維運：backup-db/restore-db）、`audit/`（分析審計：accuracy_audit/rule_audit）、`tools/`（產生器/批次：gen_taxonomy_xlsx/prejudge_reviews/translate_summaries/boundary_ab_eval+report/multi_model_eval+report/persist_multimodel_history/taxonomy_health/gen_eval_prompt_pack/eval_prompt_single/encrypt_user_secrets/dump_datapack）、
...
| `./scripts/tools/gen_eval_prompt_pack.py` | **評測交付 Prompt 包 v3 產生器**（零 LLM·唯讀 DB active 判準·模板驅動）：組裝 7 支獨立自包含 prompt——情緒傾向 ×1＋C-1~C-6 各 1。骨架層＝`config/ai_judge/prompt_templates/*.md`（Anthropic system prompt 慣例：XML 分區/critical 粗體/NEVER/If-then；`{{SLOT}}` 生成期槽位、`{TEXT}`/`{POLARITY}` 執行期佔位符；單支模板獨立成檔＝調一支不影響其他六支），判準內容一律 DB 注入（SSOT）。產 `prompts/*.md`（頁首 provenance 防手改）＋`bundle.json`＋`manifest.json`（rule_versions＋template_hashes 溯源；重生成時偵測舊包過期並警告）＋README（合併規則契約）。正式生成物入版於 `docs/prompts/`。⚠️輸出 schema 鏡射 `prejudge._attr_schema_l2_multi`——由 `backend/tests/test_prompt_pack_schema.py` 逐鍵比對護欄鎖死 | `docker exec kkday-ai-quality-backend python /app/scripts/tools/gen_eval_prompt_pack.py --out /app/tmp/prompt_pack_v3 --git-sha "$(git rev-parse --short HEAD)"`（容器需先 `docker cp` 腳本，scripts/ 非掛載；產出 `docker cp` 回 `docs/prompts/`）|
| `./scripts/tools/eval_prompt_single.py` | **單支 Prompt 評測 harness**（調適閉環驗證端）：對 7 支 prompt 逐支獨立驗證——prompt 由生成器渲染函式即時從 DB＋模板渲染（天然不過期），對 N 則已判評論（md5 穩定排序＝跨 run 可比）與 production judgments 比對。域 prompt 指標：primary 一致率/棄權正確率/命中率/多報率；polarity：極性＋sentiment 一致率。token 走 user_settings（stub 拒跑）、關 exact-cache 讀。調適閉環：改單支模板或該域規則 → `--prompt C-3 --n 20` 驗證 → 達標後重生成 `docs/prompts/` | `docker exec kkday-ai-quality-backend python /app/scripts/tools/eval_prompt_single.py --prompt C-3 --n 20 --user alvin.bian@kkday.com --out /app/tmp/eval_C-3.json` |
