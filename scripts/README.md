...
| `./scripts/tools/multi_model_eval.py` | 多模型準確度評測（唯讀不寫 attributions）：`--build-set` 建評測集（有外部 free_tag 且已初判的 reviews）／`--run --area prejudge --provider <id> --model <id>` 以指定功能區默認 + 覆寫供應商/model 逐則 `to_findings` 收集 sentiment/polarity/歸因 | `cd backend && .venv/bin/python ../scripts/tools/multi_model_eval.py --help` |
