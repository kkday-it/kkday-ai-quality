...
`ops/`（生產維運：backup-db/restore-db）、`audit/`（分析審計：accuracy_audit）、`tools/`（產生器/批次：prejudge_reviews/translate_summaries/boundary_ab_eval+report/multi_model_eval+report/persist_multimodel_history/taxonomy_health/eval_prompt_single/encrypt_user_secrets/dump_datapack）。
