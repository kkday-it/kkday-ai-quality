...
#   ./scripts/prejudge_reviews.sh --limit 2000
#   ./scripts/prejudge_reviews.sh --limit 20            # pilot
#   ./scripts/prejudge_reviews.sh --csv <path> --out <path>
...
if [ ! -x .venv/bin/python ]; then
  echo "❌ 尚無 backend/.venv；先跑 ./scripts/dev.sh 或 backend/run.sh 建環境" >&2
  exit 1
fi
