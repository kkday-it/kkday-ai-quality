...
- **啟動 / 部署**：**Docker Compose**——一鍵 `./scripts/dev/start.sh`（dev·hot reload）／生產 `docker compose up`（nginx + 多 worker）。**macOS 只需先有 Homebrew**（start.sh 自動裝並啟動 colima 引擎），PG 與所有依賴皆在容器內。
...
- **改碼即生效**（後端 `uvicorn --reload`、前端 vite HMR，掛 source volume）；首次會 build image（較久），之後秒起。**停止**：`Ctrl-C` 或另開終端跑 `./scripts/dev/stop.sh`（`--wipe` 連資料清空）。改依賴時 `docker compose -f docker-compose.dev.yml up --build` 重建。
