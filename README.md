...
> **容器引擎全自動**：start.sh 偵測不到容器工具時會**自動安裝 [colima](https://github.com/abiosoft/colima)（免費開源，大公司免授權）並啟動**——macOS 只需先有 [Homebrew](https://brew.sh)，Linux 走 apt。已有 OrbStack / Docker Desktop 者直接沿用，不強制切換。真正零手動：clone → `./scripts/dev/start.sh`。
