...
| POST | `/api/problems/export` | 啟動問題列表 xlsx 導出背景 job（1:N 多歸因合併儲存格；初判組欄含初判時間＋域/極性 Prompt 版本〔初判當時所用版本，格式「01_C-1_content v發版時間戳」〕、判決組欄含判決狀態/時間/人；欄尾附 C-1~C-6 六域命中欄〔符合/不符合·未初判留空·供篩選〕；另附「分類統計」「Prompts」〔導出當下 active 版本快照〕「說明」〔欄位語義字典〕工作表）→ {job_id}；`snapshot_model` 可選「輸出結果版本」＝該模型的 attribution_history 最新快照（未初判過的評論排除，口徑寫入統計表附註）；`compare_models` 可選「並排對比模型」多選＝基準右側每模型附一組情緒/L1/L2 對比欄（值取該模型最新快照）|
...
| GET/POST | `/api/v1/prejudge/prompt-debug/defaults` · `/prompt-debug/stream` · `/prompt-debug/prompt-versions` | 售後根因 Prompt 調試台：載入 Prompt 版本庫**最新版**（`prompts/debug/after_sales_root_cause/YYYY-MM-DD-HHMMSS.md`，一版一檔、無契約切換），任意 IM session 以 SSE 串流裁決，完成後回傳交叉欄位校驗、token 與單次 USD 估算；POST `prompt-versions` 把編輯後全文存為新版本並即刻成為線上口徑（需 `judge-rule.version.manage`）；不寫正式 attributions |
| POST/GET | `/api/v1/prejudge/prompt-debug/batch/*` | 調試台批量跑批：multipart `start`（上傳 CSV/XLSX 整批 strict 裁決，`system_prompt` 留空＝用版本庫最新版、與調試台同源；run 目錄落 `data/prompt_debug_batch/<run_id>/`）/ `runs` 列表 / `runs/{run_id}` 進度輪詢 / `cancel` 停止 / `resume` 斷點續跑（只補未成功筆；`rerun` 全部重打；manifest 鎖輸入/Prompt/schema/model）/ `files/{kind}` 下載（csv/jsonl/preds/input）。啟動/停止/續跑需 `prejudge.run` 權限 |
